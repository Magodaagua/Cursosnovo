...
<html lang="pt-br">
    <?php
        ...
        if ($row_usuario = mysqli_fetch_assoc($resultado_usuario)) {
            ...
            //$fotodoperfil = $row_usuario['FotoPerfil'];
        }
    ...
    <body data-id-user="<?php echo $id_user; ?>">
        ...
        <div class="tudo_dois">
            ...
            <div class="ladoesquerdo_dois">
                ...
                <div class="botoescomopcoes">
                    <!-- Cursos recentes -->
                    <button class="profissoes_digitais ativo" id="btn_profissoes_digitais" onclick="location.href='#cursos_recentes'">
                    ...
                    <!-- novos cursos -->
                    <button class="insights" id="btn_insights" onclick="location.href='#novos_cursos'">
                    ...
                    <a href="comunidade.php?Usuario=<?php echo $usuario;?>&Dep=<?php echo $dep;?>">
                ...
                <div class="profissoesdigitais">
                    ...
                    <div class="profissoesdigitais_titulo">
                        <div class="titulo_profissoes" id="cursos_recentes">
                            Cursos Recentes
                        </div>
                        <!-- botão -->
                        <a href="curso.php?Usuario=<?php echo $usuario;?>&Dep=<?php echo $dep;?>">
                            <button class="vermais">+ Ver Todos</button>
                        </a>
                    </div>
                    <!-- texto -->
                    <div class="texto_profissoes">Continue os cursos que começou selecionando um deles logo abaixo</div>
                    ...
                    <div class="bannersprofissoes">
                        <?php
                            // Busca no banco de dados todos os cursos mais recentes cadastrados
                            $query_cursos_recentes = "SELECT * FROM curso WHERE ID_curso IN (SELECT id_curso FROM inscricao WHERE id_usuario = $id_user) ORDER BY Datadecriacao DESC LIMIT 4"; // Obtém os 4 cursos mais recentes em que o usuário está inscrito
                            $result_cursos_recentes = mysqli_query($conn, $query_cursos_recentes);

                            if ($result_cursos_recentes) {
                                if (mysqli_num_rows($result_cursos_recentes) > 0) {
                                    while ($row = mysqli_fetch_assoc($result_cursos_recentes)) {
                                        ...
                                        $Nome2 = mb_strimwidth($row['Nome'], 0, 13 , '...');
                                        //$titulo_abreviado = mb_strimwidth($titulo, 0, 30, "...");
                                        ...
                                        echo "<div class='bannersprofissoes_nome'>{$Nome2}</b></div>";
                                        echo "</div></a>";
                                    }
                                } else {
                                    echo "<p class='retornandomensagem'>Nenhum Curso Cadastrado</p>";
                                }
                            } else {
                                echo "Erro na execução da consulta: " . mysqli_error($conn);
                            }
                ...
                <div class="novoscursosparausuario">
                    <!-- titulo -->
                    <div class="novoscursosparausuario_titulo">
                        <div class="titulo_novoscursosparausuario" id="novos_cursos">
                            Novos Cursos
                        </div>
                        <!-- botão -->
                        <a href="curso.php?Usuario=<?php echo $usuario;?>&Dep=<?php echo $dep;?>">
                    ...
                    <!-- texto -->
                    <div class="texto_profissoes">Veja os novos cursos cadastrados, não perca tempo, venha conhecer cursos recem saídos do forno </div>
                    <!--futuras atualizações
                        <div class="botoes-orbs">
                        <div class="orb orb-cheia"></div>
                        <div class="orb orb-vazia"></div>
                    ...
                    <div class="bannersprofissoes">
                        <?php
                            $query_categoria = "SELECT id FROM categoria WHERE Nome_cat = '$dep'";
                            $result_categoria = mysqli_query($conn, $query_categoria);
                            $categoria = mysqli_fetch_assoc($result_categoria);
                            $id_categoria = $categoria['id'];
     
                            // Busca no banco de dados todos os cursos mais recentes cadastrados
                            $query_cursos_recentes = "SELECT * FROM curso WHERE ID_curso NOT IN (SELECT id_curso FROM inscricao WHERE id_usuario = $id_user) AND Categoria = $id_categoria ORDER BY Datadecriacao DESC LIMIT 4";
                            $result_cursos_recentes = mysqli_query($conn, $query_cursos_recentes);
        ...
        <!-- Chat -->
        <div class="iconeChat" id="iconeChat" onclick="toggleChat()">
            <img src="../../../COMUM/img/Icons/CinzaClaro/icon-smile-white.png">
        </div>
        <!-- Inclui o chat lateral -->
        <?php include 'php/chat_lateral.php'; ?>

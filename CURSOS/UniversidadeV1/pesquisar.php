<?php 
    ...
    while($row_usuario = mysqli_fetch_assoc($resultado_usuario)){
        ...
        if($subcategoria && $subcategoria !== 'default'){
            $total_cursos_query = "SELECT COUNT(*) AS total FROM curso WHERE Nome LIKE '%$valor_pesquisar%' AND Categoria = '$id_categoria_atual' AND Subcategoria = '$subcategoria'";
            ...
            $total_cursos_query = "SELECT COUNT(*) AS total FROM curso WHERE Nome LIKE '%$valor_pesquisar%' AND Categoria = '$id_categoria_atual'";
            $resultado_total_cursos = mysqli_query($conn, $total_cursos_query);
            $row_total_cursos = mysqli_fetch_assoc($resultado_total_cursos);
            $total_de_cursos = $row_total_cursos['total'];
        }
        ...
        if($subcategoria && $subcategoria !== 'default'){
            $result_cursos = "SELECT * FROM curso WHERE Nome LIKE '%$valor_pesquisar%' AND Subcategoria = '$subcategoria'";
            $resultado_cursos = mysqli_query($conn, $result_cursos);
            $total_cursos = mysqli_num_rows($resultado_cursos);
        }
        else{
            $result_cursos = "SELECT * FROM curso WHERE Nome LIKE '%$valor_pesquisar%'";
            $resultado_cursos = mysqli_query($conn, $result_cursos);
            $total_cursos = mysqli_num_rows($resultado_cursos);
        }
...
<html lang="pt-br">
    ...
    <head>
        ...
        <title>Site de Treinamento - Pesquisa</title>
    ...
    <?php require "titulo.php"; ?>
                        ...
                        <div class="container">
                            <div class="row">
                                <?php 
                                    ...
                                    //if($valor_pesquisar != 0 OR $valor_pesquisar != ''){
                                    while ($rows_cursos = mysqli_fetch_assoc($resultado_cursos)) {
                                        // ... seu código para exibir cursos
                                        if ($rows_cursos['Categoria'] == $id_categoria_atual || empty($id_categoria_atual)) {
        ...
        <script src="javascript/filtropesquisa.js"></script>
